...
export default function BoxTab() {
  ...
  const topPicks = useMemo(
    () => applyFilter(selectTopPicks(ideas), activeFilter),
    [ideas, activeFilter],
  );

  const theBox = useMemo(
    () => applyFilter(selectBox(ideas), activeFilter),
    [ideas, activeFilter],
  );
        ...
        <>
          ...
          <section className={styles.section}>
            <h2 className={styles.sectionHeader}>
              <Heart size={20} className={styles.sectionIcon} /> Top Picks
            </h2>
            ...
            {topPicks.length === 0 && (
              <p className={styles.empty}>Heart an idea to pin it here</p>
            )}
          </section>
          ...
          <section className={styles.section}>
            <h2 className={styles.sectionHeader}>
              <Inbox size={20} className={styles.sectionIcon} /> The Box
            </h2>
            <div className={styles.cardList}>
              <AnimatePresence mode="popLayout">
                {theBox.map((idea, i) => (
            ...
            {theBox.length === 0 && (
              <p className={styles.empty}>
                The box is empty — drop an idea above!
              </p>
            )}
          </section>
        </>
